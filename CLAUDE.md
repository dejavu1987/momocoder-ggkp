...
PlatformIO/Arduino firmware for the **Momocoder GGKP** — an ESP32-S3 BLE HID combo (keyboard + mouse) with a 3x3 keypad, MPU6050 IMU "air mouse", a 64x48 SSD1306 OLED, and a Wi-Fi-remote page that fires HTTPS GETs at the companion app (`~/dejavu/momoggkp`, deployed at `momoggkp.vercel.app`). Single environment: `[env:esp32-s3]` on `esp32-s3-devkitc-1`, CPU pinned to 80 MHz (briefly bumped to 240 MHz via `cpuBumpTo()` for the Wi-Fi remote's TLS handshake and the captive-portal scan/validation), partition table `huge_app.csv`.
...
- `src/Pages.{h,cpp}` — per-page `Binding[]` tables and the `ActionKind` enum (`Key`, `MediaKey`, `MouseClick`, `ToggleScroll`, `ToggleDrag`, `NavPrev`, `NavNext`, `AdjustSens`, `AdjustDelay`, `EnterPairing`, `ForgetBonds`, `WifiRequest`, `CycleBrightness`, `ListPickerSlot/Left/Right/Confirm`). Tables are indexed by slot 0..8 in row-major order (A/UP/B / LT/OK/RT / C/DN/D), so `findBinding()` is an O(1) lookup via `slotForButton()`. `executeAction()` is the single dispatch point — every action lives in one switch; the `ListPicker*` arms route to whichever page hosts a picker via `currentListPickerView()` (today: only `Page::Wifi`).
...
- `src/WifiRemote.{h,cpp}` — connect-on-press HTTPS to `momoggkp.vercel.app`. `wifiRemoteFire(name)` runs the full associate → TLS → GET → teardown cycle (~1.2–1.6 s warm). Reads SSID/password/channel/BSSID from `wifiConfigsGetActive()` (NVS) per press; `WiFiClientSecure::setInsecure()` keeps SNI-correct routing without a CA bundle. Bumps CPU to 240 MHz only for the duration of the request via `HwUtil::cpuBumpTo/cpuRestore`.
- `src/HwUtil.{h,cpp}` — shared platform helpers used by both Wi-Fi paths: `cpuBumpTo(targetMhz, tag)` / `cpuRestore(tag)` (paired stateful CPU clock bump — single-threaded, only one bump in flight at a time, tag prefixes serial logs) and `wifiPowerOff()` (`WiFi.disconnect(true) + WiFi.mode(WIFI_OFF)` — kept Arduino-managed because raw `esp_wifi_*` shortcuts wedge the next associate with `NO_SHIELD`).
...
- While the AP is up (`WaitingForClient` / `WaitingForSubmit`), `B` toggles a full-screen QR overlay encoding `http://192.168.4.1` so the user can scan instead of typing. The QR is a static 21×21 bitmap embedded in `WifiSetup.cpp` (drawn 2× scaled inside a 48×48 white card for the quiet zone). Auto-cleared on every state transition, so any progress past WaitingForSubmit returns to the text screen.

Both Wi-Fi paths bump CPU to 240 MHz via `HwUtil::cpuBumpTo` — `wifiRemoteFire()` for the TLS handshake (RAII-shaped: bump on entry, single `cpuRestore` on exit), and `WifiSetup` across the scan and Saving validation transitions (manual pairing because the boost spans tick() calls). Teardown via `wifiPowerOff()` still logs `E (NNNN) wifi:timeout when WiFi un-init, type=4` cosmetically — bypassing wedges arduino-esp32's state machine, so we accept the warning.
...
`U8g2`, `NimBLE-Arduino`, and the `BLECombo` fork from `earthicko/ESP32-BLE-Combo`. U8g2 drives the OLED directly — the Adafruit GFX/BusIO/SSD1306 stack and the U8g2_for_Adafruit_GFX bridge were dropped once nothing referenced them. `WiFi` and `WiFiClientSecure` come bundled with the Arduino-ESP32 framework — no `lib_deps` entry needed.
...
- Per-page button behavior is data, not code: edit the appropriate `Binding[]` table in `src/Pages.cpp`. Each table is exactly 9 entries indexed by slot 0..8 (A/UP/B / LT/OK/RT / C/DN/D), so the row position in the array IS the button — there is no `BTN_*` column. Adding a new page means bumping `NUM_PAGES` in `globals.h`, adding the enum value in `Page`, adding the table + `pageDefs[]` entry in `Pages.cpp`, and (if it's a non-mouse page) the `mouseEnabled` toggle in `loop()` already handles MPU sleep automatically.
...
- A new "pick one of N" page should reuse `ListPicker` rather than rolling its own UI. Add an `enum *ItemKind` for synthetic rows (Add/Delete/etc.), build `ListPickerItem[]` with kind-tagged userIds, and dispatch from a single `onConfirm()` switch — `WifiPage.cpp` is the reference implementation. Expose the picker via a `pageGetView() -> ListPickerView*` accessor and add a `case Page::*` arm to `Pages.cpp::currentListPickerView()` so the slot/left/right actions route automatically.
