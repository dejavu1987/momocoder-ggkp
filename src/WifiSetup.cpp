...
// Toggled by BTN_B while in WaitingForClient/WaitingForSubmit so the user
// can scan a QR for http://192.168.4.1 with their phone instead of typing
// it. Auto-cleared on every state transition.
static bool showQr = false;

// QR for "http://192.168.4.1": 21x21 modules scaled 2x -> 42x42 px.
// Encoded as XBM: row-major, LSB-first per byte, padded to byte boundary.
// Drawn as black-on-white inside a 48x48 white card so the 3 px of card
// margin acts as the quiet zone scanners need.
static const uint8_t qr_setup_42[] = {
  ,
...
void wifiSetupHandleButton(int button) {
  // BTN_B in the AP-up states toggles the QR overlay for
  // http://192.168.4.1 so the user can scan instead of typing.
  if ((state == WifiSetupState::WaitingForClient ||
       state == WifiSetupState::WaitingForSubmit) &&
      button == BTN_B) {
    showQr = !showQr;
    return;
  }
...
void wifiSetupRender() {
  ...
  if (showQr &&
      (state == WifiSetupState::WaitingForClient ||
       state == WifiSetupState::WaitingForSubmit)) {
    u8g2.clearBuffer();
    // 48x48 white card centred horizontally; 3 px of card outside the QR
    // gives the quiet zone scanners want.
    u8g2.setDrawColor(1);
    u8g2.drawBox(8, 0, 48, 48);
    u8g2.setDrawColor(0);
    u8g2.drawXBM(11, 3, 42, 42, qr_setup_42);
    u8g2.setDrawColor(1);
    u8g2.sendBuffer();
    return;
  }
